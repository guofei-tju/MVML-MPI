...
def evaluate(label, output):
    ...
    acc, acc_weight = accuracy(ts, preds)
    precision = precision_score(ts, preds, average="samples")
    recall = recall_score(ts, preds, average="samples")
    f1score = f1_score(ts, preds, average="samples")
    return acc, precision, recall, f1score, acc_weight
